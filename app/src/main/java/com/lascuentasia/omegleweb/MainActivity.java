...
public class MainActivity extends Activity {
    ...
    private boolean openedGoogleLoginExternally = false;
    ...
    private boolean handleUrl(String url) {
        ...
        if (mustOpenGoogleLoginOutside(url)) {
            openedGoogleLoginExternally = true;
            Toast.makeText(this, "Inicia sesión en Google. Después vuelve a OMEGLE.", Toast.LENGTH_LONG).show();
    ...
    private boolean mustOpenGoogleLoginOutside(String url) {
    ...
    protected void onResume() {
        super.onResume();
        if (openedGoogleLoginExternally && webView != null) {
            openedGoogleLoginExternally = false;
            CookieManager.getInstance().flush();
            Toast.makeText(this, "Volviendo a OME.TV dentro de la app...", Toast.LENGTH_SHORT).show();
            webView.postDelayed(() -> webView.loadUrl(START_URL), 900);
        }
    }
